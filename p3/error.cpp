  ...
  switch (type)
  {
    ...
    case ASSIGNMENT_TYPE_ERROR:
      ...
      cerr << "Cannot assign an expression of type '" << s2
           ...
           << endl;
      break;
    case CANNOT_ASSIGN_TO_NON_MEMBER_ANIMATION_BLOCK:
      error_header();
      cerr << "Cannot assign to animation block '" << s1 << "' because it is not a member variable.  Can only assign to animation blocks that are member variables." << endl;
      break;
    ...
    case INVALID_ARGUMENT_FOR_RANDOM:
      ...
      error_header();
      cerr << "Illegal argument to random()<" << s1 << ">.  Must be >= 1.";
      if (m_runtime)
